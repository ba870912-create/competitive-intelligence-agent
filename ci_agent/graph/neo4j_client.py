...
class Neo4jClient:
    ...
    async def add_price_point(self, competitor, plan_name, price_text, date, source_url):
        """
        ...
        readings across runs and detect real changes over time, instead of
        ...
        async with self.driver.session() as session:
            await session.run(query, competitor=competitor, plan_name=plan_name,
                               price_text=price_text, date=str(date), source_url=source_url)

    async def get_price_changes(self):
        """
        For each (competitor, plan_name) pair, compares the two most recent
        PricePoint readings. Returns only pairs where the price text actually
        differs between the last two observations.
        """
        query = """
        ...
        WHERE size(recent) = 2 AND recent[0].price_text <> recent[1].price_text
